...
class Relationship
{
    ...
    {
        return $this->db->query( "SELECT * FROM $table1 INNER JOIN $table2 ON  $table1.".removePlurals( $table2 )."_id = $table2.id" )->get();
    }
    ...
    private function hasManyRelationship( $table1, $table2 )
    {
        ...
        return $this->db->query( 'SELECT * FROM :table1 INNER JOIN :combinedTable ON :table1ID = :combinedTableID1 INNER JOIN :table2 ON :table2ID = combinedTableID2',  
            [
    ...
    private function hasRelationship( $table1, $table2 ) : void
    {
        ...
        $result = $this->db->query( 'SELECT COUNT(*) as count FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = :dbname AND REFERENCED_TABLE_NAME = :table1 AND TABLE_NAME = :table2',  
            [
